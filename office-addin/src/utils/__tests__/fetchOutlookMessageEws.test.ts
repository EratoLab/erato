...
describe("fetchOutlookMessageFilesByInternetMessageIdViaEws", () => {
  ...
  it("FindItem (host) restricts on InternetMessageId then GetItem (host) resolves the MIME", async () => {
    ...
    expect(findBody).toContain('<t:DistinguishedFolderId Id="root"/>');
...
describe("fetchParentMessageInConversationViaEws", () => {
  ...
  it("returns the latest non-draft message metadata (via the hybrid host fetch)", async () => {
    // No current item: every member is a sibling served by the host transport.
    installHostMock((body) => {
      if (body.includes("<m:GetConversationItems>")) {
        return {
          status: "succeeded",
          value: conversationItemsResponse(["m1", "m2", "draft"]),
        };
      }
      if (body.includes('<t:ItemId Id="m1"/>')) {
        return {
  ...
  it("returns null (not throw) when the conversation fetch fails", async () => {
    const warnSpy = vi.spyOn(console, "warn").mockImplementation(() => {});
    installHostMock(() => ({
      status: "failed",
      error: { code: 0, message: "Bad Request" },
