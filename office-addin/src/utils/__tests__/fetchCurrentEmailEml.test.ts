...
import {
  fetchCurrentEmailEml,
  fetchCurrentEmailParsed,
} from "../fetchCurrentEmailEml";
  ...
  it("parses the Graph-returned MIME into a ParsedEmail and prefers the parsed Message-ID", async () => {
    ...
    installFetchMock((url) =>
      url.endsWith("/$value")
        ...
        : {
            ok: true,
            jsonValue: { subject: "Hi parsed", internetMessageId: "<graph-id@example.com>" },
          },
    );
  ...
  it("falls back to Graph's internetMessageId when the parsed bytes lack a Message-ID header", async () => {
    ...
    installFetchMock((url) =>
      url.endsWith("/$value")
        ...
        : {
            ok: true,
            jsonValue: { subject: "No id", internetMessageId: "<graph-id@example.com>" },
          },
    );
