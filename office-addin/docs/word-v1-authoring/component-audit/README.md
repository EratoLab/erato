...
| Errors and passive notices                                             | Use shared `Alert`. The earlier fixed-role limitation is resolved: `role="status"` supports polite notices, while errors retain `role="alert"`. The Word migration is recorded in [implementation follow-up](implementation.md#review-follow-up--22-september-2026).            |
...
- `frontend/src/components/ui/Feedback/Alert.tsx`: the fixed-role limitation observed during the audit has since been resolved by the optional `role` prop.
