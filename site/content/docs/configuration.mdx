...
`[integrations.ms_office.addin.launch_event_runtime]` declares where the handlers live and is required as soon as at least one launch event is configured. `page_path` is the HTML runtime page used by Outlook on the web, new Outlook, and Outlook on Mac; `script_path` is the JavaScript-only override used by classic Outlook on Windows. Both paths are resolved against the deployment base URL, so they have to start with `/` and must not contain `..`.

The runtime files are served by the deployment, for example from a component kit. Launch events are only rendered into the default `manifest.xml`; the Exchange Server manifest variant cannot express them.
