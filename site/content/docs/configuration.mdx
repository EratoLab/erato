...
Erato has three independently configurable audio modes. All three use the same configuration fields, but they control different user workflows:
...
**Default behavior:** All audio modes are disabled.

#### `audio_transcription`

Controls the audio attachment transcription flow.
...
Controls hands-free microphone dictation with voice activity detection, auto-send, and automatic restart after responses. This mode uses the dictation websocket protocol, but has independent provider, chunking, retry, and duration settings. Enable it separately from `audio_dictation`.
...
- **`enabled`** _(default: false)_ - Whether this audio mode is available.
- **`chat_provider_id`** _(default: None)_ - Optional provider ID to use for this audio mode. If omitted, Erato uses the first compatible audio-capable provider from the configured provider priority order.
- **`max_recording_duration_seconds`** _(default: 1200)_ - Maximum recording duration in seconds.
- **`chunk_duration_seconds`** _(default: 30)_ - Audio chunk duration in seconds. This must be greater than 0 and no larger than `max_recording_duration_seconds`.
- **`max_attempts`** _(default: 3)_ - Maximum provider attempts per chunk, including retry attempts for detected hallucination loops.
- **`initial_backoff_ms`** _(default: 250)_ - Initial retry backoff in milliseconds.
- **`max_backoff_ms`** _(default: 4000)_ - Maximum retry backoff in milliseconds.
- **`min_words_for_loop_check`** _(default: 80)_ - Minimum transcript word count before loop detection runs.
- **`min_unique_word_ratio`** _(default: 0.25)_ - Minimum unique-word ratio accepted for non-loop transcripts. Must be between `0.0` and `1.0`.
- **`max_words_per_minute`** _(default: 200)_ - Assumed maximum speech rate used to derive per-chunk output token budgets.
- **`tokens_per_word`** _(default: 1.5)_ - Token-per-word multiplier used to derive per-chunk output token budgets.
- **`output_token_buffer_factor`** _(default: 2.0)_ - Safety multiplier used to derive per-chunk output token budgets.
- **`fixed_output_token_budget`** _(default: 500)_ - Fixed output token budget added to the per-chunk heuristic.
- **`min_output_tokens`** _(default: None)_ - Optional minimum output token budget for transcription chunks.
- **`max_output_tokens`** _(default: None)_ - Optional maximum output token cap for transcription chunks. If both minimum and maximum are configured, `min_output_tokens` must be less than or equal to `max_output_tokens`.
