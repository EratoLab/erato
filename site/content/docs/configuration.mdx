...
{/* erato_toml_config_key: hidden_facets */}

Configuration for hidden facets. Hidden facets are platform-scoped, always-on system prompts that users cannot select or remove and that never appear in the facet pool. For a request whose platform matches (or for a facet declaring no platform), a hidden facet's prompt is injected once at the start of a conversation — under the same rule as the base system prompt — and then persists in history for the rest of the conversation.
...
#### `hidden_facets.facets.<id>`

{/* erato_toml_config_key: hidden_facets.facets.<facet-id> */}

Map of hidden facet configurations, keyed by facet id.
...
Each hidden facet supports these fields:

{/* erato_toml_config_key: hidden_facets.facets.<facet-id>.platform */}
{/* erato_toml_config_key: hidden_facets.facets.<facet-id>.additional_system_prompt */}
{/* erato_toml_config_key: hidden_facets.facets.<facet-id>.additional_system_prompt.source */}
{/* erato_toml_config_key: hidden_facets.facets.<facet-id>.additional_system_prompt.prompt */}
{/* erato_toml_config_key: hidden_facets.facets.<facet-id>.additional_system_prompt.prompt_name */}
{/* erato_toml_config_key: hidden_facets.facets.<facet-id>.additional_system_prompt.label */}
{/* erato_toml_config_key: hidden_facets.facets.<facet-id>.additional_system_prompt.fallback */}

- `platform` - Optional platform identifier (e.g. `"outlook"`), matched against the request's `X-Erato-Platform`. Omit to apply on all platforms.
- `additional_system_prompt` - The baseline system prompt to inject. A prompt source: either an inline string, or a Langfuse-backed prompt via `source` / `prompt` / `prompt_name` / `label` / `fallback`.
