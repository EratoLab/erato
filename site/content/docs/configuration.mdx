...
- **`system_prompt`** - Optional prompt source specification (string or object)
...
**Note:** This option is mutually exclusive with `system_prompt_langfuse` (deprecated). Requires the [Langfuse integration](./integrations/langfuse) to be enabled when using `source = "langfuse"`.
...
This prompt is used as the instruction prompt for generating chat summaries and supports prompt source formats (string or object), including template placeholders from our system prompt renderer.
...
- `prompt` - Default prompt source specification
- `localized_prompts` - Optional map of locale code to prompt source specification
