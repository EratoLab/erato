...
**Default behavior:** `heartbeat_interval_secs` defaults to `10`, `stale_after_secs` to `30`, `terminal_retention_secs` to `60`, `shutdown_drain_secs` to `25`, and `provider_idle_timeout_secs` to `300`.
...
provider_idle_timeout_secs = 300
```
...
Seconds a turn may wait on its chat provider without receiving anything — no response headers, no chunk — before the generation is failed and reported as a provider error.
...
The default leaves ample room for a reasoning model's think time before its first token. Set to `0` to wait on the provider indefinitely.
