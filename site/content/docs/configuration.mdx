...
#### `delegation.tasks.max_tool_calls_per_task`

{/* erato_toml_config_key: delegation.tasks.max_tool_calls_per_task */}

Tool-call budget for a single task run. A task child gets its own budget, independent of the origin turn's. When unset, it inherits `generation.max_tool_calls_per_message`.

**Default value:** unset (inherits `generation.max_tool_calls_per_message`)
...
{/* erato_toml_config_key: experimental_facets.facets.<facet-id>.delegation.max_tool_calls_per_task */}
...
- `max_tool_calls_per_task` - Overrides `delegation.tasks.max_tool_calls_per_task`.
