...
{/* erato_toml_config_key: assistants.delegation.enabled */}
...
#### `assistants.delegation.max_concurrent_background_runs`

{/* erato_toml_config_key: assistants.delegation.max_concurrent_background_runs */}

Maximum number of a user's delegated background runs that may be in flight at once. A background delegation attempted while the cap is reached is refused as a tool-call error, which the model reports in prose; the turn itself still completes. A single message is also capped at this many background launches. The count is read over live generation leases without locking, so it is a soft bound — concurrent dispatches can briefly overshoot it. Set it to `0` to disable the cap. Only relevant when `assistants.delegation.allow_background` is enabled.
...
```toml
[assistants.delegation]
max_concurrent_background_runs = 10
```
...
{/* erato_toml_config_key: assistants.delegation.max_mentions_per_message */}
...
#### `assistants.delegation.result_max_chars`

{/* erato_toml_config_key: assistants.delegation.result_max_chars */}

Maximum number of characters of a delegate's final message that are returned to the origin chat. Longer results are truncated and marked as such in the tool result envelope.
...
{/* erato_toml_config_key: assistants.delegation.auto_archive_after_days */}

Number of days without activity after which a finished delegated run is archived, whether or not the chat that spawned it was ever archived. Delegated runs are hidden from the chat listing, so without this they would accumulate indefinitely. Archiving the chat that spawned a run also archives the run right away, unless the run has not finished yet.

Archived chats are deleted after `cleanup_archived_max_age_days`, so this option requires `cleanup_enabled`. Set it to `0` to never archive a delegated run automatically. A run is also left alone once it is pinned, shared through a share link, or written into by the user.
...
#### `assistants.delegation.preamble`
