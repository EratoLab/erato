...
EXE/MSI downloads. With all PEM fields unset, bootstrap TLS is disabled. Configure
one complete pair: fixed `certificate_pem` / `private_key_pem`, or
...
bootstrap files do not have that slot limit. macOS/Linux downloads currently have
no injection transport; installers can supply an adjacent `bootstrap.json`.
