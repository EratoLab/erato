...
When `backend.loadBalancer.enabled` is true, the chart injects the upstream and
reverse-proxy settings through environment variables so authenticated requests
automatically pass through the intermediate load balancer. The load balancer
uses the original `X-Forwarded-For` client address for consistent routing.
