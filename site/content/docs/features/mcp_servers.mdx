...
A marked output field must be part of an object that also contains a sibling `mime_type` or `content_type` string. The MCP tool result must provide the object as JSON in text content.
...
3. Uses the annotated sibling value as the display name, or generates a name when the annotation or value is absent.
...
MCP-provided display names are not used as storage paths. At most one sibling field may be marked as the file name for a file-content field.

The `x-chat.erato/file_content_field` and `x-chat.erato/file_name_field` aliases are also accepted for compatibility. File-output processing currently supports image MIME types only.
