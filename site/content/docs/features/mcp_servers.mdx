...
Erato extends MCP tool input and output schemas with annotations for transferring files without putting their Base64 contents into the model context.

In the future, file transfers are planned to use the [SEP-2631 mechanism](https://github.com/modelcontextprotocol/modelcontextprotocol/pull/2631) once it has been accepted.

#### File-content fields

Mark a string field containing Base64-encoded file data with `chat.erato/file_content_field`:
...
`contentEncoding` must be `base64` for input fields. It should also be specified for output fields to accurately describe their representation.

The annotations can be used in nested object properties, arrays, local schema references, and `oneOf`, `anyOf`, or `allOf` schemas.

#### File inputs

For a marked input field, the model works with an Erato file URI instead of Base64 data:

```text
erato-file://<file_upload_id>
```

Before exposing the tool to the model, Erato adds this URI requirement to the field description and removes the custom annotations that model providers may not support. When the model calls the tool, Erato resolves the URI to an accessible file upload, reads the file from its configured storage provider, Base64-encodes it, and sends the resulting string to the MCP server.

The MCP server therefore continues to receive the Base64 value declared by its original input schema; the URI is only used between the model and Erato.

#### File outputs

A marked output field must be part of an object that also contains a sibling `mime_type` or `content_type` string. The MCP tool result must provide the object in `structuredContent` or as JSON in text content. When both are present, `structuredContent` takes precedence.

An optional sibling string field can supply the file's display name by carrying the `chat.erato/file_name_field` annotation. The field name itself is arbitrary:
...
For each marked output, Erato:

1. Decodes and stores the Base64 data.
2. Creates a file upload associated with the current chat.
3. Uses the annotated sibling value as the display name. Without a file-name annotation, a sibling `name` value is used; if no name is provided, Erato generates one.
4. Replaces the Base64 value in the tool result with an `erato-file://<file_upload_id>` URI.
5. Adds a file pointer to the assistant message so the client can render the file.

MCP-provided display names are not used as storage paths. Names containing path separators, colons, or control characters, and the names `.` and `..`, are rejected. Invalid Base64 and unsafe filenames fail the tool call with a recoverable error before any files in the result are stored. At most one sibling field may be marked as the file name for a file-content field.

The `x-chat.erato/file_content_field` and `x-chat.erato/file_name_field` aliases are also accepted for compatibility. File outputs support all MIME types, including DOCX and PDF, and arrays may mix images and documents. The original MIME type is passed to file storage. Images use the image renderer; other files appear as attachment cards with the same preview options as user uploads. Documents supported by text extraction remain available in subsequent model context. Files without an available preview or text extractor can still be downloaded.
...
# Disabled by default for backwards compatibility. When enabled, the
# annotation policy determines whether an MCP tool call pauses for approval.
[mcp_servers_global.approval]
enabled = false
preset = "permissive" # "permissive" or "restrictive"
allow_always = false
...
`[mcp_servers_global.approval]` controls the global MCP approval gate and is disabled by default for backwards compatibility. With `preset = "permissive"`, a tool is paused only when its MCP `destructiveHint` is `true` (or absent). With `preset = "restrictive"`, a tool proceeds without a prompt only when it explicitly declares `readOnlyHint = true`, `destructiveHint = false`, and `openWorldHint = false`.

`openWorldHint` represents the likelihood that a tool can send data outside the local trust boundary: permissive mode permits an explicitly non-destructive open-world tool, whereas restrictive mode prompts before it runs. `idempotentHint` remains retry metadata; it never changes the first-call approval decision. Missing annotations use pessimistic MCP defaults, so they require approval under both presets.

Set `allow_always = true` to expose an **Always allow** choice. The resulting user setting is scoped to the user, MCP server ID, and tool name; it can be deactivated through the MCP approval settings API. Disabling `allow_always` hides the choice and prevents those persisted settings from bypassing future gates.
...
#### OAuth resource selection

`mcp_servers.<server-id>.authentication.oauth2.resource` controls the OAuth
`resource` parameter consistently in authorization, authorization-code exchange,
and token refresh. The MCP endpoint URL and the API resource identifier can be
different, including different hosts (for example with Microsoft Entra).

| Configuration | Behavior |
| --- | --- |
| `resource` unset | Use `resource` from Protected Resource Metadata (PRM). If PRM or its resource field is absent, use the MCP endpoint URL without its query string or fragment. |
| `resource = "https://api.example.com"` | Use this exact identifier, taking precedence over PRM. |
| `resource = ""` | Omit the parameter entirely, even when PRM declares a resource. Use for providers that require resource selection through scopes alone. |

Nonempty resource identifiers must be absolute URIs without fragments. PRM and
explicit identifiers are sent verbatim, preserving trailing slashes and resource
query parameters. Transport query parameters do not affect PRM selection and are
excluded from the endpoint fallback. Invalid resource metadata fails automatic
selection; an explicit override or omission bypasses that metadata value.
Authorization-server discovery and scope selection still run in every mode.

For example, an MCP endpoint at `https://mcp.example.com/mcp` can advertise
`https://api.example.com` as its PRM resource and
`https://api.example.com/Mcp.Read` as its scope. Automatic selection uses that API
identifier. To supply it explicitly when metadata is unavailable or incorrect:
...
The override selects the resource; it does not rewrite configured scopes. For
Entra, choose scopes qualified by the intended API's resource identifier. Servers
with no PRM keep the endpoint fallback, except that transport query strings and
fragments are removed. Reconnect an existing authorization after changing its
resource or scopes so the user grants access to the intended API.
...
- Authorization-server metadata must declare its issuer, as required by rmcp 3.2.0. Erato forwards the optional callback `iss` parameter for issuer validation.
