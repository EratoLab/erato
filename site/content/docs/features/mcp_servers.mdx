...
Erato currently supports three MCP authentication modes.
...
Forward credentials from the current user session to the MCP server. This supports forwarding either the user's access token or the OIDC ID token that authenticated the Erato session. The selected token is sent as `Authorization: Bearer <token>`.
