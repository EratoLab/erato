...
value to declare, and the host renders its own component for that point while
the kit's other overrides stay installed. Whether the host actually falls back
or only logs is one constant, `COMPONENT_KIT_VERSION_STANCE`, which ships as
`"warn"` while deployed kits are still undeclared.
