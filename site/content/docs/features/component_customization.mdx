...
value to declare. What happens next is one constant,
`COMPONENT_KIT_VERSION_STANCE`. It ships as `"warn"`, which reports the
mismatch and installs the override anyway while deployed kits are still
undeclared; `"enforce"` makes the host render its own component for that point
instead, leaving the kit's other overrides installed.
...
The suite checks that the required menu ids are present, in their required
relative order, with the required disabled and confirmation flags. A kit is
free to insert its own items anywhere and to give any item its own icon or
label — those are not what the cases read. Ids the host gates OUT for a row
must stay out: an item rendered inert is worse than one that is missing.
