...
The main frontend loads a small React runtime entrypoint first, then component
kit entrypoints, then the main app entrypoint. Component kits can assume
`window.ERATO_REACT` is set when their entrypoint evaluates and should use that
host React singleton rather than bundling a separate React copy. The example
package in `component-kit-example/` registers all current extension points and
builds to the expected `index-<hash>.js` plus `style.css` layout.
