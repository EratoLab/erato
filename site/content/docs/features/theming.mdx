...
- `data-ui="list-row"` (a row in a plain vertical list of choices outside a menu and outside the sidebar, such as the assistant mention browser; it also carries the class `list-row-geometry`. A row that only frames a control it does not own — a checkbox whose click belongs to the checkbox — is not one of these and has no hook yet)
