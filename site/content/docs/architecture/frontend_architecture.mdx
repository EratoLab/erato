...
For deeper UI customization without modifying core code, the frontend exposes a component registry. Deployments can load component kits at runtime, and custom frontend builds can still register compiled-in overrides.

- Registry location: `frontend/src/config/componentRegistry.ts`
- Runtime kits: subdirectories under `frontend.component_kits.directory`, served from `/public/component-kits/<kit-name>/`
- Custom build components: `frontend/src/customer/components/`
...
- Runtime component kits can be deployed independently of the main frontend bundle, while compiled-in overrides still require rebuilding the frontend.
