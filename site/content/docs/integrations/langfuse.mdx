...
- **Summary Tracing**: Optionally records generated chat summaries to Langfuse; this can be disabled separately via `summary_tracing_enabled` when summary trace noise is too high.
...
summary_tracing_enabled = true  # optional, defaults to true
enable_feedback = true  # optional, defaults to false
use_otel = false  # optional, defaults to false
```
...
By default, Erato sends Langfuse tracing data to the legacy Langfuse ingestion endpoint. To send trace and observation data via Langfuse's OpenTelemetry endpoint instead, enable `use_otel`:
...
When `use_otel = true`, Erato derives the OTLP trace endpoint from `base_url` and sends tracing data to `${base_url}/api/public/otel/v1/traces` using OTLP over HTTP. The same Langfuse public and secret keys are used for authentication.

This flag affects trace and observation transport only. User feedback forwarding still uses Langfuse score ingestion and remains controlled by `enable_feedback`.
...
6. **Control Summary Traces** (optional): Set `summary_tracing_enabled = false` to keep summary traces off while preserving completion traces.
7. **Enable Feedback Forwarding** (optional): Set `enable_feedback = true` to forward user feedback to Langfuse
8. **Set Up Prompt Management** (optional): Create prompts in Langfuse and reference them in your chat provider configuration
