...
test.describe("Message ID Lifecycle Investigation", () => {
  test(
    ...
    async ({ page }) => {
      ...
      page.on("console", (msg) => {
        if (
          msg.text().includes("[MESSAGE_ID]") ||
          msg.text().includes("temp-user") ||
          msg.text().includes("0198eb")
        ) {
      ...
      for (let i = 0; i < testMessages.length; i++) {
        ...
        console.log(
          `[MESSAGE_LIFECYCLE] Sending message ${i + 1}: "${messageContent}"`,
        );
        ...
        console.log(
          `[MESSAGE_LIFECYCLE] After sending message ${i + 1}, found ${messageCount} user messages`,
        );
        ...
        for (let j = 0; j < messageCount; j++) {
          ...
          if (messageId) {
            ...
            console.log(
              `[MESSAGE_LIFECYCLE] Message ${j + 1}: ID="${messageId}", Content="${content?.slice(0, 50)}..."`,
            );
          }
        }
        ...
        console.log(
          `[MESSAGE_LIFECYCLE] Waiting for potential ID replacement...`,
        );
        ...
        console.log(
          `[MESSAGE_LIFECYCLE] Before wait: [${currentMessageIds.join(", ")}]`,
        );
        console.log(
          `[MESSAGE_LIFECYCLE] After wait:  [${updatedMessageIds.join(", ")}]`,
        );

        // Check if any temp IDs were replaced
        for (
          let j = 0;
          j < Math.min(currentMessageIds.length, updatedMessageIds.length);
          j++
        ) {
          if (currentMessageIds[j] !== updatedMessageIds[j]) {
            console.log(
              `[MESSAGE_LIFECYCLE] ✅ ID REPLACEMENT: ${currentMessageIds[j]} → ${updatedMessageIds[j]}`,
            );
        ...
        console.log(
          `[MESSAGE_LIFECYCLE] ====== Completed message ${i + 1} ======`,
        );
      }
      ...
      console.log(
        `[MESSAGE_LIFECYCLE] Total messages tracked: ${messageTracker.length}`,
      );
      ...
      console.log(
        `[MESSAGE_LIFECYCLE] Final message count in UI: ${finalCount}`,
      );

      // Capture final state
      const finalMessageData: Array<{
        id: string;
        content: string;
        isTemp: boolean;
      }> = [];
      ...
      for (let i = 0; i < finalCount; i++) {
        const messageId = await finalUserMessages
          .nth(i)
          .getAttribute("data-message-id");
        ...
        if (messageId && content) {
          ...
          console.log(
            `  ID: ${messageId} ${isTemp ? "(TEMP - NOT REPLACED!)" : "(REAL UUID - REPLACED!)"}`,
          );
      ...
      finalMessageData.forEach((msg, index) => {
        if (msg.isTemp) {
          tempIdCount++;
          console.log(
            `[MESSAGE_LIFECYCLE] ❌ Message ${index + 1} stuck with temp ID: ${msg.id}`,
          );
        } else {
          realIdCount++;
          console.log(
            `[MESSAGE_LIFECYCLE] ✅ Message ${index + 1} has real ID: ${msg.id}`,
          );
        }
      });
      ...
      console.log(
        `[MESSAGE_LIFECYCLE] Messages with real IDs: ${realIdCount}/${finalCount}`,
      );
      console.log(
        `[MESSAGE_LIFECYCLE] Messages stuck with temp IDs: ${tempIdCount}/${finalCount}`,
      );

      // Hypothesis testing
      if (tempIdCount === 0) {
        console.log(
          `[MESSAGE_LIFECYCLE] 🎯 HYPOTHESIS: All IDs replaced correctly - no bug!`,
        );
      } else if (realIdCount === 1 && tempIdCount > 0) {
        console.log(
          `[MESSAGE_LIFECYCLE] 🎯 HYPOTHESIS: Only first message gets real ID - race condition or sequential processing bug!`,
        );
      } else if (tempIdCount === finalCount) {
        console.log(
          `[MESSAGE_LIFECYCLE] 🎯 HYPOTHESIS: No IDs ever get replaced - complete replacement mechanism failure!`,
        );
      } else {
        console.log(
          `[MESSAGE_LIFECYCLE] 🎯 HYPOTHESIS: Partial replacement - complex race condition or timing issue!`,
        );
      }
  ...
  test(
    ...
    async ({ page }) => {
      ...
      console.log(
        `[RACE_TEST] Testing FAST message sending (potential race condition)`,
      );
      ...
      for (let i = 0; i < fastCount; i++) {
        ...
        if (messageId) {
          ...
          console.log(
            `[RACE_TEST] Fast message ${i + 1}: ${messageId} ${isTemp ? "(TEMP)" : "(REAL)"}`,
          );
        }
      }
      ...
      console.log(
        `[RACE_TEST] Testing SLOW message sending (no race condition)`,
      );
      ...
      for (let i = 0; i < slowCount; i++) {
        const messageId = await slowUserMessages
          .nth(i)
          .getAttribute("data-message-id");
        if (messageId) {
          const isTemp = messageId.startsWith("temp-user-");
          ...
          console.log(
            `[RACE_TEST] Slow message ${i + 1}: ${messageId} ${isTemp ? "(TEMP)" : "(REAL)"}`,
          );
        }
      }
      ...
      console.log(
        `[RACE_TEST] Fast sending - temp IDs: ${fastTempCount}/${fastResults.length}`,
      );
      console.log(
        `[RACE_TEST] Slow sending - temp IDs: ${slowTempCount}/${slowResults.length}`,
      );

      if (fastTempCount > slowTempCount) {
        console.log(
          `[RACE_TEST] 🎯 CONCLUSION: Race condition detected! Fast sending causes more temp IDs to persist.`,
        );
      } else if (fastTempCount === slowTempCount && fastTempCount > 0) {
        console.log(
          `[RACE_TEST] 🎯 CONCLUSION: Not a race condition - systematic replacement failure regardless of speed.`,
        );
      } else {
        console.log(
          `[RACE_TEST] 🎯 CONCLUSION: No clear race condition pattern detected.`,
        );
      }
