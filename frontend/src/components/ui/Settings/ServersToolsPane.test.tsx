...
describe("MCP authorization notices in server settings", () => {
  it("offers a real recovery link when opening the browser returns null, without restarting the wait", async () => {
    ...
    expect(screen.getByRole("status")).toHaveAttribute("data-ui", "alert");
  ...
  it("does not render an orphan notice and removes its runtime state", () => {
  ...
  it("removes a success notice when newer server data contradicts it", () => {
  ...
  it("does not let the success timer cancel a subsequent authorization", async () => {
    servers([{ ...sales, connection_status: "SUCCESS" }]);
    useMcpAuthorizationStore.setState({ phases: { sales: "connected" } });
    render(tree());
