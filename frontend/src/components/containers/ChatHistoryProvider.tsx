...
import { useChats, useMessages } from "../../lib/generated/v1betaApi/v1betaApiComponents";
...
export const ChatHistoryProvider: React.FC<ChatHistoryProviderProps> = ({
  ...
  const [sessions, { set, remove }] = useMap<Record<string, ChatSession>>(
    initialSessions.reduce((acc, session) => ({
      ...acc,
      [session.id]: session
    }), {})
  );
  ...
  const setCurrentSessionId = useCallback((newId: string | null) => {
