        ...
        let next_state = match state {
            SseAutoReconnectStreamStateProj::Connected { stream } => {
                match ready!(stream.poll_next(cx)) {
                        ...
                        if let Some(data) = sse.data {
                            match serde_json::from_str::<ServerJsonRpcMessage>(&data) {
                                Err(error) => {
                                    ...
                                    tracing::debug!(last_event_id=%last_id, "failed to deserialize server message: {error}");
                    ...
                    Some(Err(error)) => {
                        this.connector
                            .handle_stream_error(&error, this.last_event_id.as_deref());
                        let retrying = this
                            .connector
            ...
            SseAutoReconnectStreamStateProj::Retrying {
                ...
                match retry_result {
                    ...
                    Err(error) => {
                        tracing::debug!("retry sse stream error: {error}");
                        *retry_times += 1;
                        if let Some(interval) = this.retry_policy.retry(*retry_times) {
                            ...
                            tracing::error!("sse stream error: {error}, max retry times reached");
                            this.state.set(SseAutoReconnectStreamState::Terminated);
                            return Poll::Ready(Some(Err(error)));
                        }
                    }
            ...
            SseAutoReconnectStreamStateProj::Terminated => return Poll::Ready(None),
        };
