...
pub const FORWARDED_ACCESS_TOKEN_AUTH_HEADER_TEMPLATE: &str = "{{prefix}}{{access_token}}";

type PlaceholderFn = for<'a> fn(&McpForwardedAccessTokenContext<'a>) -> String;

#[derive(Clone, Debug)]
pub struct McpAccessTokenAuthHeaderRenderer {
    placeholder_registry: HashMap<String, PlaceholderFn>,
}

impl Default for McpAccessTokenAuthHeaderRenderer {
    fn default() -> Self {
        Self::new()
    }
}

impl McpAccessTokenAuthHeaderRenderer {
    pub fn new() -> Self {
        let mut renderer = Self {
            placeholder_registry: HashMap::new(),
        };

        renderer.register_placeholder("access_token", render_access_token);
    ...
    pub fn render<'a>(&self, template: &str, ctx: &McpForwardedAccessTokenContext<'a>) -> String {
        ...
        match handlebars.render_template(template, &Value::Object(context_values)) {
            ...
            Err(error) => {
                tracing::debug!(
                    error = %error,
                    template = %template,
                    "Failed to render MCP access token auth header template with Handlebars, returning raw template"
                );
                template.to_string()
            }
        }
...
pub(crate) fn render_access_token(ctx: &McpForwardedAccessTokenContext) -> String {
    ctx.access_token.unwrap_or("").to_string()
}

pub(crate) fn render_prefix(ctx: &McpForwardedAccessTokenContext) -> String {
...
mod tests {
    ...
    #[test]
    fn test_access_token_with_prefix() {
    ...
    #[test]
    fn test_access_token_default_prefix() {
        let renderer = McpAccessTokenAuthHeaderRenderer::new();
        let result = renderer.render(
            FORWARDED_ACCESS_TOKEN_AUTH_HEADER_TEMPLATE,
            &McpForwardedAccessTokenContext {
