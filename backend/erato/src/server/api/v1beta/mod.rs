...
async fn stream_multipart_field_with_limit(
    ...
    while let Some(chunk) = field.chunk().await.map_err(|e| {
        ...
        if size_bytes + chunk.len() > max_upload_size {
            ...
            drain_remaining_multipart_fields(multipart).await?;
...
async fn drain_remaining_multipart_fields(multipart: &mut Multipart) -> Result<(), StatusCode> {
    // Continue consuming the multipart stream until EOF so the upstream proxy can finish writing
    // the request body even when the backend has already decided to reject the upload.
    while let Some(mut remaining_field) = multipart.next_field().await.map_err(|e| {
        tracing::error!(
            "Failed while draining remaining multipart fields after size limit: {}",
            e
        );
        StatusCode::BAD_REQUEST
    })? {
        drain_multipart_field(&mut remaining_field).await?;
    }
