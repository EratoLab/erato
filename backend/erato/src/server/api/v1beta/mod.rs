...
use crate::server::api::v1beta::message_streaming::{
    ...
    AbortStreamResponse, ActionFacetRequest, AlreadyContinuedError, ClientToolResultRequest,
    ClientToolResultResponse, EditMessageRequest, EditMessageStreamingResponseMessage,
    GenerationRunningError, MessageSubmitRequest, MessageSubmitStreamingResponseMessage,
    NothingToReactError, ReactToTaskResultRequest, ResumeStreamRequest, abort_message_stream,
    client_tool_result, continue_message_sse, edit_message_sse, message_submit_sse,
    react_to_task_result_sse, regenerate_message_sse, resume_message_sse,
};
...
pub struct ChatDetail {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    #[schema(nullable = false)]
    adopted_at: Option<DateTime<FixedOffset>>,
...
pub struct ChatMessage {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    #[schema(nullable = false)]
    mcp_tools_disabled_by_user: Option<Vec<String>>,
...
impl ChatMessage {
    ...
    pub fn from_model_with_feedback(
        ...
        let mcp_tools_disabled_by_user = generation_metadata
            .as_ref()
            .and_then(|metadata| metadata.mcp_tools_disabled_by_user.clone());
...
async fn extend_recent_chats_to_api_model(
    ...
    for (chat, file_uploads_result) in model_chats.into_iter().zip(file_uploads_results) {
        ...
        api_chats.push(RecentChat {
            ...
            disabled_mcp_tools: chat.disabled_mcp_tools.clone(),
        ...
        .map(|(fa, can_edit)| {
            ...
            let api_assistant = AssistantWithFiles {
                assistant: Assistant {
                    ...
                    enforce_facet_settings: fa.assistant.enforce_facet_settings,
...
pub async fn create_chat(
    ...
    let assistant_id = if let Some(assistant_id_str) = assistant_id {
...
#[utoipa::path(
    ...
    params(
        ("chat_id" = String, Path, description = "The ID of the chat to retrieve")
    ),
    responses(
        (status = OK, body = ChatDetail, description = "Successfully retrieved the chat"),
        (status = BAD_REQUEST, description = "Invalid chat ID format"),
        (status = NOT_FOUND, description = "When the chat does not exist or is not accessible"),
        (status = INTERNAL_SERVER_ERROR, description = "Server error")
    ),
...
pub async fn chat_detail(
    ...
    let chat =
        models::chat::get_chat_detail(&app_state.db, &policy, &me_user.to_subject(), &chat_id)
            .await
            .map_err(|error| {
    ...
    let can_edit = permissions::can_user_edit_chat(&me_user.id, &chat.owner_user_id);
...
#[utoipa::path(
    ...
    path = "/me/chats/{chat_id}",
    params(
        ("chat_id" = String, Path, description = "The ID of the chat to update")
    ),
...
pub async fn update_chat(
    ...
    let update = async {
        ...
        if let Some(disabled_mcp_tools) = disabled_mcp_tools {
            updated_chat = Some(
                update_chat_disabled_mcp_tools(
                    &app_state.db,
                    &policy,
                    &subject,
                    &chat_id,
    ...
    let title_resolved = resolve_chat_display_name(
        ...
        updated_chat.title_by_summary.as_deref(),
    );

    Ok(Json(UpdateChatResponse {
        ...
        disabled_mcp_tools: updated_chat.disabled_mcp_tools,
    }))
}
