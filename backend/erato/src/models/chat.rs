    ...
    format!(
        r#"CASE
                WHEN ("chats"."assistant_configuration" #>> '{{provenance,kind}}') IS DISTINCT FROM 'delegation'
                    ...
                    OR EXISTS (SELECT 1 FROM local_delegation_jobs j WHERE j.chat_id="chats".id AND j.generation_id="chats".active_generation_id AND j.state IN ({ACTIVE_JOB_STATES_SQL}))
                THEN NULL
...
/// Replace the set of MCP servers the user switched off for a chat.
pub async fn update_chat_disabled_mcp_server_ids(
    conn: &DatabaseConnection,
    policy: &PolicyEngine,
    subject: &Subject,
    chat_id: &Uuid,
...
pub(crate) fn generation_unfinished_condition(alias: &str, param_index: u8) -> String {
    format!(
        r#"COALESCE(
            ...
            OR EXISTS (SELECT 1 FROM local_delegation_jobs j WHERE j.chat_id={alias}.id AND j.generation_id={alias}.active_generation_id AND j.state IN ({ACTIVE_JOB_STATES_SQL}))
            OR (
