...
#[test]
fn test_prompt_guardrail_config_and_provider_override() {
    let mut temp_file = Builder::new()
        .suffix(".toml")
        .tempfile()
        .expect("Failed to create temporary file");
...
[guardrails.prompt_patterns.leak_data]
type = "regex"
pattern = "leak\\s+all\\s+company\\s+data"
language = "en"
tags = ["input"]

[chat_providers]
priority_order = ["mock", "override"]
...
[chat_providers.providers.override]
provider_kind = "openai"
model_name = "override-model"

[chat_providers.providers.override.guardrails.filter_input_prompt_injection]
enabled = true
filter_pattern_ids = ["ignore_previous"]
filter_pattern_tags = []
...
"#;
    ...
    assert_eq!(config.guardrails.prompt_patterns.len(), 2);

    let mock_guardrails = config.chat_provider_guardrails(Some("mock"));
    assert!(mock_guardrails.filter_input_prompt_injection.enabled);
    assert_eq!(
        mock_guardrails
...
fn test_prompt_guardrail_invalid_regex_is_rejected() {
    let mut temp_file = Builder::new()
        .suffix(".toml")
        .tempfile()
        .expect("Failed to create temporary file");
    let config_content = r#"
[guardrails.prompt_patterns.invalid]
type = "regex"
pattern = "["
tags = ["input"]
...
#[test]
fn test_user_preferences_enabled_can_be_disabled() {
    let mut temp_file = Builder::new()
        .suffix(".toml")
        .tempfile()
        .expect("Failed to create temporary file");
...
#[test]
fn test_assistants_context_warning_threshold_can_be_configured() {
    let mut temp_file = Builder::new()
        .suffix(".toml")
        .tempfile()
        .expect("Failed to create temporary file");
...
context_file_contributor_threshold = 0.02
"#;
    ...
    assert_eq!(config.assistants.context_warning_threshold, 0.1);
    assert_eq!(config.assistants.context_file_contributor_threshold, 0.02);
}

#[test]
fn test_deprecated_experimental_assistants_key_still_parses() {
    let mut temp_file = Builder::new()
        .suffix(".toml")
        .tempfile()
        .expect("Failed to create temporary file");
...
#[test]
#[should_panic(expected = "Invalid assistants configuration")]
fn test_assistants_context_warning_threshold_must_be_between_zero_and_one() {
    let mut temp_file = Builder::new()
        .suffix(".toml")
        .tempfile()
        .expect("Failed to create temporary file");
...
"#,
    ...
    // The task route carries no coupling to `assistants.enabled`. The two tool
    // budgets are independent literals - there is no total to reconcile them
    // against and no inheritance from the generation cap.
    assert_eq!(config.delegation.tasks.max_server_tool_calls_per_task, 5);
    assert_eq!(config.delegation.tasks.max_client_tool_calls_per_task, 30);
}
...
"#,
    ...
    assert_eq!(inline.max_client_tool_calls_per_task, Some(40));
    assert_eq!(inline.max_server_tool_calls_per_task, None);
    assert_eq!(
        inline.persona,
    ...
    // A facet may raise the client budget while leaving the server budget
    // alone: the two are overridden independently.
    assert_eq!(config.delegation.tasks.max_server_tool_calls_per_task, 5);
}
