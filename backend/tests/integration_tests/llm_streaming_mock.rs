...
async fn test_message_submit_with_content_filter_triggered(pool: Pool<Postgres>) {
    ...
    mocks.mock(|when, then| {
        ...
        then.status(http::StatusCode::BAD_REQUEST)
    ...
    println!("Error event data: {}", serde_json::to_string_pretty(&error_event_data).unwrap());
    ...
    assert!(
        error_description.contains("content") || error_description.contains("filter") || error_description.contains("policy"),
    ...
    let filter_details = error_event_data.get("filter_details")
        .expect("Error event should have filter_details for content_filter errors");
    
    println!("Filter details: {}", serde_json::to_string_pretty(&filter_details).unwrap());
    ...
    let message_error_obj = message_error.unwrap().as_object()
        .expect("Message error should be an object");
    
    let message_error_type = message_error_obj.get("error_type")
        .and_then(|v| v.as_str())
