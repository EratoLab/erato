...
use crate::{log, matcher::ImageMock, request_id::RequestId, responses::build_image_generation_response};
...
pub async fn generate_images(
    ...
    let response = if let Some(matched_image) = match_image_mock(&image_request.prompt, &image_mocks) {
        log::log_with_id(
            request_id.as_str(),
