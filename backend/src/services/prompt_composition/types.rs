use crate::models::message::{ContentPartImage, GenerationInputMessages, InputMessage};
...
pub enum AbstractChatSequencePart {
    ...
    /// Reference to a previous user message in the chat history
    PreviousUserMessage { message_id: Uuid },

    /// Reference to a previous assistant message in the chat history
    PreviousAssistantMessage { message_id: Uuid },

    /// The current user input being submitted
    CurrentUserInput {
        content: String,
        file_ids: Vec<Uuid>,
    ...
    AssistantFile { file_id: Uuid, filename: String },
}
