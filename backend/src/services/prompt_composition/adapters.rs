...
use crate::models::message::{ContentPartImage, ContentPartText};
...
impl<'a> MessageRepository for DatabaseMessageRepository<'a> {
    async fn get_message_by_id(&self, message_id: &Uuid) -> Result<messages::Model, Report> {
        crate::models::message::get_message_by_id(
            self.conn,
            self.policy,
            self.subject,
            message_id,
    ...
    async fn get_generation_input_messages(
        ...
        // This traverses messages backward from previous_message_id
        // We'll need to extract this logic or create a new function
        // For now, we can use the existing get_message_history_from_db
...
impl<'a> AppStateFileResolver<'a> {
    fn get_sharepoint_context(&self) -> Option<SharepointContext> {
