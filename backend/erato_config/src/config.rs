...
pub struct AppConfig {
    ...
    #[serde(default)]
    pub assistant_hub: AssistantHubConfig,
...
pub struct DelegationTasksConfig {
    ...
    // How many SERVER-executed tool calls one task run may make. These are
    // MCP calls: the backend holds the session, the connection and a slot in
    // the sequential tool loop, so they scale with shared capacity rather than
    // with the user's machine. Independent of the client budget below - neither
    // constrains the other, and the two are never summed into a shared total.
    // `0` forbids server-executed tools in task runs entirely.
    // Defaults to `5`.
    #[serde(default = "default_delegation_tasks_max_server_tool_calls_per_task")]
    pub max_server_tool_calls_per_task: u32,

    // How many CLIENT-executed tool calls one task run may make, including
    // work a client forwards to the desktop sidecar. These run on the user's
    // own device, so a deployment can afford to be far more generous here than
    // with the server budget. They still cost one model round trip each.
    // `0` forbids client-executed tools in task runs entirely.
    // Defaults to `30`.
...
impl Default for DelegationTasksConfig {
    fn default() -> Self {
        Self {
            ...
            max_server_tool_calls_per_task: default_delegation_tasks_max_server_tool_calls_per_task(
            ),
...
pub struct AssistantHubConfig {
    ...
    // Categories available for assistant hub submissions.
    //
...
#[derive(Debug, Default, Deserialize, PartialEq, Eq, Clone, Facet)]
pub struct AssistantHubReviewerPermissionsConfig {
    // Map of rule name to reviewer rule configuration.
    #[serde(default)]
    pub rules: HashMap<String, AssistantHubReviewerRule>,
}
...
#[serde(tag = "rule_type")]
#[repr(C)]
pub enum AssistantHubReviewerRule {
    #[serde(rename = "allow-for-group-members")]
    #[facet(rename = "allow-for-group-members")]
    AllowForGroupMembers {
        // List of group identifiers whose members can review assistant hub submissions.
        groups: Vec<String>,
    },
}

#[derive(Debug, Deserialize, PartialEq, Eq, Clone, Facet)]
pub struct AssistantHubCategoryConfig {
    ...
    pub fn validate(&self) -> Result<(), Report> {
        ...
        for (category_id, category) in &self.categories {
            if category_id.trim().is_empty() {
                return Err(eyre!("assistant_hub category IDs must not be empty"));
            }

            if category.display_name.trim().is_empty() {
                return Err(eyre!(
                    "assistant_hub category '{}' must have a non-empty display_name",
                    category_id
                ));
            }
            ...
            if category.icon.trim().is_empty() {
                return Err(eyre!(
                    "assistant_hub category '{}' must have a non-empty icon",
                    category_id
                ));
            }
        }
...
impl AssistantHubReviewerPermissionsConfig {
    pub fn validate(&self) -> Result<(), Report> {
        for (rule_name, rule) in &self.rules {
            if let Err(e) = rule.validate() {
                return Err(eyre!(
                    "Invalid configuration for assistant hub reviewer rule '{}': {}",
                    rule_name,
                    e
                ));
            }
...
impl AssistantHubReviewerRule {
    pub fn validate(&self) -> Result<(), Report> {
        match self {
            AssistantHubReviewerRule::AllowForGroupMembers { groups } => {
                if groups.is_empty() {
                    return Err(eyre!(
    ...
    pub fn allows_review(&self, user_groups: &[String]) -> bool {
        match self {
            AssistantHubReviewerRule::AllowForGroupMembers { groups } => user_groups
                .iter()
...
impl Default for GenerationStatusConfig {
    fn default() -> Self {
        Self {
            ...
            provider_idle_timeout_secs: default_generation_status_provider_idle_timeout_secs(),
        }
    }
}
